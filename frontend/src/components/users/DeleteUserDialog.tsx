...
export function DeleteUserDialog({ user, onClose, onConfirm }: DeleteUserDialogProps) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setError(
        deleteError instanceof Error ? deleteError.message : 'Não foi possível excluir o usuário.',
      );
      setIsDeleting(false);
    }
  }
  ...
  return (
    <Modal title="Excluir usuário" onClose={onClose} size="sm">
      <div className="px-5 py-5 sm:px-6">
        <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-rose-50 text-rose-600">
          <AlertTriangle size={20} />
        </span>
        <p className="mt-4 text-sm leading-6 text-slate-600">
          Tem certeza de que deseja remover <strong className="text-slate-900">{user.name}</strong>{' '}
          do workspace?
        </p>
        <p className="mt-2 rounded-xl bg-amber-50 px-3.5 py-3 text-xs leading-5 text-amber-800">
          As tarefas atribuídas continuarão existindo, mas ficarão sem responsável.
        </p>
